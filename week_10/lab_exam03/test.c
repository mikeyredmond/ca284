...
get rid of white space and punctuation and then check if the string is a palindrome or not. It eventually returns and frees up memory used.
*/
...
int main(int argc)
{
    ...
    check(fstring, nstring); // freeing up memory used to hold data in the program
    if (fstring != NULL) {
        free(fstring);
    }
